(() => {
  ...
  function details(id) {
    ...
    if(item.time){element('h3','事件时间',panel);element('p',`${item.time.start||'未知'}${item.time.end?' — '+item.time.end:''} · ${item.time.precision} · ${item.time.certainty}`,panel);}
    if(item.validity){element('h3','有效期与核验',panel);element('p',`${item.validity.start||'未知'} — ${item.validity.end||'未知'}\n状态：${item.validity.state} · 核验：${item.validity.verified_at||'未知'}`,panel);}
    if(item.body){const body=element('section',undefined,panel,'body-text');prose(item.body,body);}
    for(const claim of item.claims||[]){const c=element('div',undefined,panel,'claim');element('small',`${claim.kind} · ${claim.certainty||'unknown'}`,c);prose(claim.text,c);for(const cite of claim.citations){recordButton(cite.source_id,c,`${cite.role} · `);element('small',cite.locator||'来源整体',c);}}
  ...
  function select(id) {if(!nodes.has(id))return;selected=id;expanded=new Set([id,...adjacency.get(id)]);if(nodes.get(id).type==='source'){$('sources').checked=true;filterTypes.add('source');}details(id);$('focus').disabled=false;$('results').hidden=true;location.hash=encodeURIComponent(id);refresh();}
  function refresh(){
  ...
  function draw(){scheduled=false;ctx.setTransform(dpr,0,0,dpr,0,0);ctx.clearRect(0,0,width,height);
    ...
    for(const n of shown){const p=screen(n);if(p.x<-180||p.x>width+100||p.y<-30||p.y>height+30)continue;const active=n.id===selected||n.id===hover;const radius=active?7:n.type==='source'?3:4.5;ctx.globalAlpha=selected&&!active&&!near.has(n.id)?.42:1;if(active){ctx.beginPath();ctx.arc(p.x,p.y,12,0,Math.PI*2);ctx.fillStyle='#e3eadb';ctx.fill();}ctx.beginPath();ctx.arc(p.x,p.y,radius,0,Math.PI*2);ctx.fillStyle=colors[n.type];ctx.fill();if(active||near.has(n.id)||shown.length<=70||camera.scale>1.8){ctx.fillStyle='#3c5145';const title=n.label.length>32?n.label.slice(0,31)+'…':n.label;ctx.fillText(title,p.x+radius+6,p.y);}}
    ctx.globalAlpha=1;
  }
  function fit(){if(!shown.length){camera={x:0,y:0,scale:1};requestDraw();return;}const xs=shown.map(n=>n.x),ys=shown.map(n=>n.y),minx=Math.min(...xs),maxx=Math.max(...xs),miny=Math.min(...ys),maxy=Math.max(...ys);const usableWidth=Math.max(240,width-180),usableHeight=Math.max(170,height-(width<580?250:210));camera.scale=Math.max(.04,Math.min(2,usableWidth/Math.max(maxx-minx,100),usableHeight/Math.max(maxy-miny,100)));camera.x=-(minx+maxx)/2*camera.scale;camera.y=-(miny+maxy)/2*camera.scale+(width<580?25:0);requestDraw();}
