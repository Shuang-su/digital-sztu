...
GraphQL 查询已无法继续缩小，或收到无法解析的成功响应时，暂时性错误最多额外重试三次，依次等待 5、15、45 秒。重试使用同一查询与分页游标；每次请求仍检查存储空间及额度。`--progress` 会在标准错误输出 `discovery-retry` 事件，包含尝试次数和等待秒数，最终标准输出仍只有一个 JSON 文档。认证失败和限流不重试；服务端明确提供 `Retry-After` 时保存等待边界并退出。Ctrl+C 或空间不足保留已提交页；重试耗尽后仍返回错误及恢复命令，不将暂时失败的页面标记为完成。
